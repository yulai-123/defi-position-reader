...
![资产范围与问题边界](../diagrams/asset-scope.png)
...
![协议资产解析流程](../diagrams/position-resolution-flow.png)
...
这种注册式的 Adapter 结构可以让协议接入保持相对独立。当前项目通过代码静态注册协议 Adapter；后续接入真实协议时，可以按 `protocols/{protocol-id}` 目录维护各自的协议描述、metadata 同步和用户仓位读取逻辑。实际业务场景还会多不少细节内容，比如如何维护代币行情、如何对外提供接口服务、如何展示每个资产的穿透计算链路等等。
