...
func TestLiveAccountFixturesAreValid(t *testing.T) {
	...
	for _, fixture := range fixtures {
		...
		if strings.TrimSpace(fixture.Strategy) == "" {
			t.Fatalf("%s: strategy is empty", fixture.ID)
		}
		if !common.IsHexAddress(fixture.Owner) {
			t.Fatalf("%s: owner is not a valid EVM address: %q", fixture.ID, fixture.Owner)
		}
		if !strings.HasPrefix(fixture.DebankPool, "https://debank.com/") {
			t.Fatalf("%s: debank pool must be a DeBank URL: %q", fixture.ID, fixture.DebankPool)
		}
...
func TestLiveAaveV3PositionsFromFixtures(t *testing.T) {
	if os.Getenv("DPR_LIVE_TESTS") != "1" {
		t.Skip("set DPR_LIVE_TESTS=1 to run Aave V3 live RPC smoke tests")
	}
	...
	for _, fixture := range fixtures {
		...
		t.Logf("%s: positions=%d types=%v symbols=%v", fixture.ID, len(positions), types, symbols)
		if fixture.UnsupportedReason != "" {
			t.Logf("%s: unsupported fixture note: %s", fixture.ID, fixture.UnsupportedReason)
			continue
		}
		if len(positions) == 0 {
